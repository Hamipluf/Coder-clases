...
import { mockingProduct } from "../utils/mocks.js";
...
// Genera 100 producto y lo manda en la respuesta
export const mockProducts = async (req, res) => {
  const products = [];
  try {
    for (let i = 0; i < 100; i++) {
      const data = await mockingProduct();
      products.push(data);
    }
    res.json(products);
  } catch (error) {
    console.log(error);
    res.status(500).send("No se pudo generar el producto");
  }
};
